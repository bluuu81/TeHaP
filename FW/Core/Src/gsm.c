...
bool  Gsm_Ussd(char *send,char *receive)
{
  ...
  {
    if(receive != NULL) memcpy(receive, Sim80x.Gsm.Msg, strlen(Sim80x.Gsm.Msg));
    return true;
  }
...
//######################################################################################################
bool  Gsm_GetMyNumber(void)
{
  uint8_t answer;
  answer = Sim80x_SendAtCommand("AT+CNUM\r\n",1000,1,"\r\n+CNUM:");
  if((answer == 1) && (Sim80x.Gsm.MyNumber[0]!=0))
    return true;
  else
    return false;
}
